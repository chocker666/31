...
bool compare_pca(std::vector <Eigen::Matrix3f> a, std::vector <Eigen::Matrix3f> b){
return a.size()>b.size();}
bool compare_center(std::vector <Eigen::Vector4f> a, std::vector <Eigen::Vector4f> b){
return a.size()>b.size();}
bool compare_main_center(Eigen::Vector4f a, Eigen::Vector4f b){
return a(2) < b(2);}

int len_compn = 0, dep_compn = 0, wid_compn = 0;

int main (int argc, char **argv){
//STEP A:dilution; B:boundary; C:macro seg; D:remove wall; E:stair sample; F:pca sample; G:pca stair; H:length line
char STEP = 'K';
if(argv[1] == NULL){ROS_INFO("\nneed target(.pcd) name. \n");return 1;}
if(argv[2] != NULL)STEP = argv[2][0];


std::ofstream log("/home/u/log.txt", ios::app);
...
pcl::io::loadPCDFile("/home/u/Desktop/"+(std::string)argv[1]+".pcd", *origin);
...
//**1**A**input sample dilution
clock_t cal_start = clock();

clock_t now = clock();
vg.setInputCloud(origin);
vg.setLeafSize(VX0, VX0, VX0);
vg.filter(*cloud_filtered);
pcl::copyPointCloud(*cloud_filtered, *origin);
log << setprecision(3) << (double)(clock() - now) / CLOCKS_PER_SEC << "	";
if(STEP == 'A')
ROS_INFO("vg finished: %d left, in %f s",origin->width,  (double)(clock() - now) / CLOCKS_PER_SEC);

if(STEP != 'K')ROS_INFO("voxel( %d ) | spend %lf s",origin->width,  (double)(clock() - cal_start) / CLOCKS_PER_SEC);//end~~~~~~~~~~~~~~~~~~~
if(STEP == 'A'){
pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> single_color(origin, 0, 255, 0);
viewer->addPointCloud<pcl::PointXYZ> (origin, single_color, "sample cloud");
...
//**2**B**preEC
cal_start = clock();
std::vector<pcl::PointCloud<pcl::PointXYZ> > AP;
std::vector<pcl::PointIndices> pre_indices;
pcl::EuclideanClusterExtraction<pcl::PointXYZ> prec;
prec.setClusterTolerance (0.1);
prec.setMinClusterSize (0.4 * origin->width);
prec.setMaxClusterSize (origin->width);
prec.setSearchMethod (tree);
prec.setInputCloud (origin);
prec.extract (pre_indices);
AP.resize(pre_indices.size());
for (j=0;j<pre_indices.size();j++){
  for(std::vector<int>::const_iterator t = pre_indices[j].indices.begin();t!=pre_indices[j].indices.end (); ++t)
    AP[j].points.push_back (origin->points[*t]);
  AP[j].width = AP[j].points.size ();
  AP[j].height = 1;
  AP[j].is_dense = true;
  if(STEP == 'B'){
    std::cout << pre_indices[j].indices.size() << endl;
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cluster = AP[j].makeShared();
...
if(STEP != 'K')ROS_INFO("pre-EC in %d clusters | spend %lf s", (int)pre_indices.size(),  (double)(clock() - cal_start) / CLOCKS_PER_SEC);//end~~~~~~~~~~~~~~~~~~~
