...
{  // Allocate a new shared pointer for zero-copy sharing with other nodelets.
  rslidar_msgs::rslidarScan_bpPtr scan(new rslidar_msgs::rslidarScan_bp);
  ...
  if (config_.cut_angle >= 0)  // Cut at specific angle feature enabled
  {
    ...
    rslidar_msgs::rslidarPacket_bp tmp_packet;
    while (true)
    {
      while (true)
      {
  ...
  else  // standard behaviour
  {
    ...
    if (time_synchronization_)
    {
      ...
      rslidar_msgs::rslidarPacket_bp pkt = scan->packets[0];
      struct tm stm;
...
{
  // reading and publishing scans as fast as possible.
  rslidar_msgs::rslidarPacket_bpPtr difop_packet_ptr(new rslidar_msgs::rslidarPacket_bp);
  while (ros::ok())
  {
    // keep reading
    rslidar_msgs::rslidarPacket_bp difop_packet_msg;
